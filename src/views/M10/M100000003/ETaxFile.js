...
import { CREATE_FILE_M100000003, DOWNLOAD_FILE_M100000003 } from "../../../service/api/report";
import FormDefault from "../../../components/form/FormDefault/FormDefault";

const { Text } = Typography;
const dateFormat = "DD/MM/YYYY";

const ETaxFile = (props) => {
  ...
  const fields = [
    ...
    },
    {
      ...
      option: {
        ...
        rules: [{ required: true, message: "กรุณาเลือกวันที่ !" }],
        initialValue: _isEmpty(initialValue) ? moment("00:00:00", "HH:mm:ss") : initialValue.endDate
      },
    }
  ];
  ...
  const handleDownload = async (item) => {
    if (item.pathName) {
      ...
      try {
        await DOWNLOAD_FILE_M100000003(sendItem, props.auth.token);
      } catch (error) {
        console.log(error);
      }
    } else {
  ...
  const getDataList = async (dataOutput) => {
    try {
      setLoading(true);
      const res = await CREATE_FILE_M100000003(dataOutput, props.auth.token);
      console.log("res 8.11", res);

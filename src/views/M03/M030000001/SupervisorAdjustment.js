...
import { header31 } from "../../../tools/excel/header";
import { footer31 } from "../../../tools/excel/footer";
...
const SupervisorAdjustment = (props) => {
  ...
  const getDataInfo = async (data = null) => {
    try {
      setLoading(true);
      const res = await GET_DATA_INFO_M030000001(data, props.auth.token);
      if (res.status.code === "S200") {
        setLoading(false);
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: ["3.1 Supervisor Adjustment Report"],
  });

...
import { GET_DATA_INFO_M030000003 } from "../../../service/api/report";
...
const DailyViolationReports = (props) => {
  ...
  const getDataInfo = async (data = null) => {
    try {
      setLoading(true);
      const res = await GET_DATA_INFO_M030000003(data, props.auth.token);
      if (res.status.code === "S200") {
        setDataSource(res);
        setLoading(false);
      } else {
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: ["3.3 Violation Summary Report"]
  });
      ...
      props: {
        ...
        onClick: () =>
          _exportFileExcel({
            ...
            fileName: "3.3 Violation Summary Report",
            header: header33,
            footer: footer33,
          }),
        // disabled: dataSource.list.length < 1,
        ...
        <div className="d-none">
          <PrintReport
            ...
            propsHeader={{
              ...
              headerText,
              TopicText: "3.3 รายละเอียดรายงานสรุปการการฝ่าฝืนรายวัน",
            }}
            columnPerPage={header33.length}
            footer={footer33}
          />
        </div>
      </div>

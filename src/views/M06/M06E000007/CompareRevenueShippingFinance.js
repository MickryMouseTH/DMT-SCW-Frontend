...
import { GET_DATA_INFO_M06E000007, GET_DATA_EXPORT_M06E000007 } from "../../../service/api/report";
import { getTSBList_API } from "../../../service/api/util";
import moment from "moment";
import { header67E } from "../../../tools/excel/header";
import { footer67E } from "../../../tools/excel/footer";
...
const CompareRevenueShipping = (props) => {
    ...
    const columns = [
        ...
        },
        {
            ...
            children: [
                {
                    title: "DOH",
                    dataIndex: "amountEasyPassBank",
                    key: "amountEasyPassBank",
                    align: "center",
                    render: (text, record) => (
                        <div
                ...
                {
                    title: "SOD",
                    dataIndex: "amountEasyPassSod",
                    key: "amountEasyPassSod",
                    align: "center",
                    render: (text, record) => (
                        <div
                            style={{ textAlign: "right", cursor: "pointer" }}
                ...
                {
                    title: "ต่าง",
                    dataIndex: "amountEasyPassDiff",
                    key: "amountEasyPassDiff",
                    align: "center",
                    render: (text, record) => (
                        <div
                            style={{ textAlign: "right", cursor: "pointer" }}
        ...
        if (item.seckey) {
            try {
                await props.history.push({
                    pathname: `/reports/compare-revenue-shipping-companies-with-system-finance-epass/tod-detail/${item.seckey}`,
                    value: initialValue,
                });
            } catch (error) {
                console.log(error);
    ...
    const getDataInfo = async (data = null) => {
        try {
            setLoading(true);
            const res = await GET_DATA_INFO_M06E000007(data, props.auth.token);
            if (res.status.code === "S200") {
                setLoading(false);
                setDataSource(res);
    ...
    const handlePrint = useReactToPrint({
        ...
        documentTitle: ["6.7.5 Compare Revenue Shipping Companies With System (EasyPass)"],
    });
            ...
            props: {
                ...
                onClick: () =>
                    _exportFileExcel({
                        ...
                        fileName: "6.7.5 Compare Revenue Shipping Companies With System (EasyPass)",
                        header: header67E,
                        footer: footer67E,
                    }),
                // disabled: dataSource.list.length < 1,
    ...
    const ExportToSap = async () => {
        ...
        setLoading(true);
        try {
            const res = await GET_DATA_EXPORT_M06E000007(datainfo, props.auth.token)
            if (res.status.code === "S200") {
                Swal.fire({
                    icon: "success",
                    title: "สำเร็จ",
        ...
        <Skeleton loading={loading} active>
            ...
            <div className="d-none">
                <PrintReport
                    ...
                    header={header67E}
                    footer={footer67E}
                    columnPerPage={header67E.length}
                    propsHeader={{
                        headerText,
                        position: "d-flex justify-content-start",
                        colSpan: 12,
                        TopicText: "6.7.5 เปรียบเทียบรายได้ กับ ข้อมูลจจากระบบ (EasyPass) "
                    }}

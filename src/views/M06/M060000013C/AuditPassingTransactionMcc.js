...
import {
  GET_DATA_INFO_M060000013C, POST_SAVE_M060000013C
...
const AuditPassingTransactionMcc = (props) => {
  ...
  const getDataInfo = async (data = null) => {
    try {
      setLoading(true);
      const res = await GET_DATA_INFO_M060000013C(data, props.auth.token);
      if (res.status.code === "S200") {
        setLoading(false);
  ...
  const save = async (data) => {
    try {
      ...
      const res = await POST_SAVE_M060000013C(
        body,
        props.auth.token
      );
      if (res.status.code === "S200") {

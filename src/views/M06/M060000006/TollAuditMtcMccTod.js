...
import { GET_DATA_TOLL_AUDIT_MTC_MCC_TOD_M060000006 } from "../../../service/api/report";
...
const TollAuditMtcMccTod = (props) => {
  ...
  const columnsMccTodTotal = ["totalMcc", "totalTod", "totalDif"];
  ...
  const getDataTollAuditMtcMccTod = async (shift) => {
    try {
      setLoading(true);
      const body = {
        seckey: `${props.match.params.id}`,
      };
      const res = await GET_DATA_TOLL_AUDIT_MTC_MCC_TOD_M060000006(
        body,
        props.auth.token
      );
  ...
  const handleOnFinish = (value) => {
    ...
    getDataTollAuditMtcMccTod(shift);
  };
  ...
  return (
    <Skeleton loading={loading} active>
      ...
      <Row>
        <h3
          style={{ marginTop: -30 }}
        >{` รายการนำส่งเงินเทียบกับยอดจากระบบ (MCC-TOD) ${
            ...
            return (
              <>
                <Table.Summary.Row>
                  ...
                  {tableSummaryRowRender(columnsMccTodTotal)}
                </Table.Summary.Row>
              </>
            );
          }}
...
export default connect(mapStateToProps, mapDispatchToProps)(TollAuditMtcMccTod);

...
import { GET_DATA_TOLL_AUDIT_MTC_MCC_SOD_M060000006 } from "../../../service/api/report";
...
const TollAuditMtcMccSod = (props) => {
  ...
  const columnsMccSodTotal = ["totalMcc", "totalSod", "totalDif"];
  ...
  const getDataTollAuditMtcMccSod = async (shift) => {
    try {
      setLoading(true);
      const body = {
        seckey: `${props.match.params.id}`,
      };
      const res = await GET_DATA_TOLL_AUDIT_MTC_MCC_SOD_M060000006(
        body,
        props.auth.token
      );
      if (res.status.code === "S200") {
        ...
        if (shift) {
          ...
          list.forEach((element) => {
            totalMcc += element.mcc;
            totalSod += element.sod;
            totalDif += element.dif;
          });
          res.list = list;
          res.totalMcc = totalMcc;
  ...
  const handleOnFinish = (value) => {
    ...
    getDataTollAuditMtcMccSod(shift);
  };
  ...
  return (
    <Skeleton loading={loading} active>
      ...
      <Row>
        <h3
          style={{ marginTop: -30 }}
        >{` รายการนำส่งเงินเทียบกับยอดจากระบบ (MCC-SOD) ${
            ...
            return (
              <>
                <Table.Summary.Row>
                  ...
                  {tableSummaryRowRender(columnsMccSodTotal)}
                </Table.Summary.Row>
              </>
            );
          }}
...
export default connect(mapStateToProps, mapDispatchToProps)(TollAuditMtcMccSod);

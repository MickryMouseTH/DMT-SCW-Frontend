...
import {
  GET_DATA_INFO_M060000015A, POST_SAVE_M060000015A
...
const AuditPassingTransactionExcept = (props) => {
  ...
  const getDataInfo = async (data = null) => {
    try {
      setLoading(true);
      const res = await GET_DATA_INFO_M060000015A(data, props.auth.token);
      if (res.status.code === "S200") {
        setLoading(false);
  ...
  const save = async (data) => {
    try {
      ...
      const res = await POST_SAVE_M060000015A(
        body,
        props.auth.token
      );
      if (res.status.code === "S200") {

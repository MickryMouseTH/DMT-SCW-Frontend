...
import { GET_DATA_INFO_M06D000007, GET_DATA_EXPORT_M06D000007 } from "../../../service/api/report";
import { getTSBList_API } from "../../../service/api/util";
import moment from "moment";
import { header67D } from "../../../tools/excel/header";
import { footer67D } from "../../../tools/excel/footer";
...
const CompareRevenueShipping = (props) => {
        ...
        if (item.seckey) {
            try {
                await props.history.push({
                    pathname: `/reports/compare-revenue-shipping-companies-with-system-finance-mpass/tod-detail/${item.seckey}`,
                    value: initialValue,
                });
            } catch (error) {
                console.log(error);
    ...
    const getDataInfo = async (data = null) => {
        try {
            setLoading(true);
            const res = await GET_DATA_INFO_M06D000007(data, props.auth.token);
            if (res.status.code === "S200") {
                setLoading(false);
                setDataSource(res);
    ...
    const handlePrint = useReactToPrint({
        ...
        documentTitle: ["6.7.4 Compare Revenue Shipping Companies With System (MPass)"],
    });
            ...
            props: {
                ...
                onClick: () =>
                    _exportFileExcel({
                        ...
                        fileName: "6.7.4 Compare Revenue Shipping Companies With System (MPass)",
                        header: header67D,
                        footer: footer67D,
                    }),
                // disabled: dataSource.list.length < 1,
    ...
    const ExportToSap = async () => {
        ...
        setLoading(true);
        try {
            const res = await GET_DATA_EXPORT_M06D000007(datainfo, props.auth.token)
            if (res.status.code === "S200") {
                Swal.fire({
                    icon: "success",
                    title: "สำเร็จ",
        ...
        <Skeleton loading={loading} active>
            ...
            <div className="d-none">
                <PrintReport
                    ...
                    header={header67D}
                    footer={footer67D}
                    columnPerPage={header67D.length}
                    propsHeader={{
                        headerText,
                        position: "d-flex justify-content-start",
                        colSpan: 12,
                        TopicText: "6.7.4 เปรียบเทียบรายได้ กับ ข้อมูลจจากระบบ (MPass) "
                    }}

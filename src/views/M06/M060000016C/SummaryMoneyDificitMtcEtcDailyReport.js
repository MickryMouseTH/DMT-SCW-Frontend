...
import { GET_DATA_INFO_M060000016C, GET_DATA_INFO_M060000016C_getPaymentmethodListAPI } from "../../../service/api/report";
import {
  getTSBList_API,
...
const SummaryMoneyDificitMtcEtcDailyReport = (props) => {
  ...
  const getPaymentmethodList = async () => {
    try {
      setLoading(true);
      const res = await GET_DATA_INFO_M060000016C_getPaymentmethodListAPI(null, props.auth.token);
      if (res.status.code === "S200") {
  ...
  const addIndex = (res) => {
    ...
    const totalAll = {
      date: 'TOTAL',
      mtcClass1Trx: res.mtcClass1TrxTotal,
      mtcClass2Trx: res.mtcClass2TrxTotal,
      mtcClassTotalTrx: res.mtcClassTotalTrxTotal,
      etcTrx: res.etcTrxTotal,
      totalTrx: res.totalTrxTotal,
      mtcClass1Revenue: res.mtcClass1RevenueTotal,
      mtcClass2Revenue: res.mtcClass2RevenueTotal,
      mtcClassTotalRevenue: res.mtcClassTotalRevenueTotal,
      etcRevenue: res.etcRevenueTotal,
      totalRevenue: res.totalRevenueTotal,
    }
    return {
      ...res, list: list,
  ...
  const getDataInfo = async (data = null) => {
    ...
    try {
      setLoading(true);
      const res = await GET_DATA_INFO_M060000016C(data, props.auth.token);
      if (res.status.code === "S200") {
        setLoading(false);
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: ["6.16.3 รายงานสรุปยอดรายได้และปริมาณจราจร"],
  });
  ...
  const headerExcel = [
    ...
    { name: "ปริมาณรถที่เก็บเงินได้ (คัน) MTC ป 1", key: "mtcClass1Trx", type: "nullColumn", align: 'center', className: 'text-right' },
    { name: "ปริมาณรถที่เก็บเงินได้ (คัน) MTC ป 2", key: "mtcClass2Trx", type: "nullColumn", align: 'center', className: 'text-right' },
    { name: "ปริมาณรถที่เก็บเงินได้ (คัน) MTC รวม", key: "mtcClassTotalTrx", type: "nullColumn", align: 'center', className: 'text-right' },
    { name: "ปริมาณรถที่เก็บเงินได้ (คัน) ETC รวม", key: "etcTrx", type: "nullColumn", align: 'center', className: 'text-right' },
    { name: "ปริมาณรถที่เก็บเงินได้ (คัน) รวมทั้งสิ้น", key: "totalTrx", type: "nullColumn", align: 'center', className: 'text-right' },
    { name: "จำนวนเงิน (บาท) MTC ป 1", key: "mtcClass1Revenue", type: "nullColumn", align: 'center', className: 'text-right' },
    { name: "จำนวนเงิน (บาท) MTC ป 2", key: "mtcClass2Revenue", type: "nullColumn", align: 'center', className: 'text-right' },
    { name: "จำนวนเงิน (บาท) MTC รวม", key: "mtcClassTotalRevenue", type: "nullColumn", align: 'center', className: 'text-right' },
    { name: "จำนวนเงิน (บาท) ETC รวม", key: "etcRevenue", type: "nullColumn", align: 'center', className: 'text-right' },
    { name: "จำนวนเงิน (บาท) รวมทั้งสิ้น", key: "totalRevenue", type: "nullColumn", align: 'center', className: 'text-right' },
  ]
      ...
      props: {
        ...
        onClick: () => {
          _exportFileExcel({
            ...
            fileName: "6.16.3 รายงานสรุปยอดรายได้และปริมาณจราจร",
            header: headerExcel,
          });
        },
      },
      ...
      <div>
                ...
                <>
                  <Table.Summary.Row className="bg_default">
                    ...
                    <Table.Summary.Cell colSpan={1} className="text-right" index={1}>
                      <Text>{_isNull(Number(dataSource.mtcClass1TrxTotal))}</Text>
                    </Table.Summary.Cell>
                    <Table.Summary.Cell colSpan={1} className="text-right" index={2}>
                      <Text>{_isNull(Number(dataSource.mtcClass2TrxTotal))}</Text>
                    </Table.Summary.Cell>
                    <Table.Summary.Cell colSpan={1} className="text-right" index={3}>
                      <Text>{_isNull(Number(dataSource.mtcClassTotalTrxTotal))}</Text>
                    </Table.Summary.Cell>
                    <Table.Summary.Cell colSpan={1} className="text-right" index={4}>
                      <Text>{_isNull(Number(dataSource.etcTrxTotal))}</Text>
                    </Table.Summary.Cell>
                    <Table.Summary.Cell colSpan={1} className="text-right" index={5}>
                      <Text>{_isNull(Number(dataSource.totalTrxTotal))}</Text>
                    </Table.Summary.Cell>
                    <Table.Summary.Cell colSpan={1} className="text-right" index={6}>
                      <Text>{_isNull(Number(dataSource.mtcClass1RevenueTotal))}</Text>
                    </Table.Summary.Cell>
                    <Table.Summary.Cell colSpan={1} className="text-right" index={7}>
                      <Text>{_isNull(Number(dataSource.mtcClass2RevenueTotal))}</Text>
                    </Table.Summary.Cell>
                    <Table.Summary.Cell colSpan={1} className="text-right" index={8}>
                      <Text>{_isNull(Number(dataSource.mtcClassTotalRevenueTotal))}</Text>
                    </Table.Summary.Cell>
                    <Table.Summary.Cell colSpan={1} className="text-right" index={9}>
                      <Text>{_isNull(Number(dataSource.etcRevenueTotal))}</Text>
                    </Table.Summary.Cell>
                    <Table.Summary.Cell colSpan={1} className="text-right" index={10}>
                      <Text>{_isNull(Number(dataSource.totalRevenueTotal))}</Text>
                    </Table.Summary.Cell>
                  </Table.Summary.Row>
                </>
              );
        ...
        <div className="d-none">
          <PrintPDF
            ...
            HeaderBar={{
              headerText,
              TopicText: "6.16.3 รายงานสรุปยอดรายได้และปริมาณจราจร"
            }}
          />
        </div>
      </div>
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(SummaryMoneyDificitMtcEtcDailyReport);

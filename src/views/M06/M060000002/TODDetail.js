...
import { GET_DATA_TOD_DETAIL_M060000002 } from "../../../service/api/report";
import { GET_DATA_ADJUST_SAVE_M060000002 } from "../../../service/api/report";
import { header62Detail } from "../../../tools/excel/header";
import { footer62Detail } from "../../../tools/excel/footer";
...
const TODReports = (props) => {
  ...
  const getDataTodDetail = async () => {
    try {
      ...
      const res = await GET_DATA_TOD_DETAIL_M060000002(
        body,
        props.auth.token
      );
  ...
  const goBack = () => {
    props.history.push({
      pathname: `/reports/compare-revenue-shipping-companies-with-system`,
  ...
  const adjustSave = async (data) => {
    try {
      ...
      const res = await GET_DATA_ADJUST_SAVE_M060000002(
        body,
        props.auth.token
      );
      if (res.status.code === "S200") {
      ...
      props: {
        ...
        onClick: () =>
          _exportFileExcel({
            ...
            header: header62Detail,
            footer: footer62Detail,
          }),
        // disabled: dataSource.list.length < 1,
      },
    },
      ...
      <div>
        ...
        <div className="d-none">
          <PrintReport
            ...
            header={header62Detail}
            footer={footer62Detail}
            columnPerPage={header62Detail.length}
            rowPerPage={20}
            propsHeader={{
              headerText,
              position: "d-flex justify-content-start",
              colSpan: 12,
              TopicText: "6.2 เปรียบเทียบรายได้ บริษัทขนส่ง กับ ข้อมูลจจากระบบ (Audit)"
            }}

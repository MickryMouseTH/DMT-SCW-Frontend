...
import {
  GET_DATA_INFO_M060000013A, POST_FARECLASS_M060000013A, POST_SAVE_M060000013A
...
const AuditPassingTransactionSodTrx = (props) => {
  ...
  const getDataInfo = async (data = null) => {
    try {
      setLoading(true);
      const res = await GET_DATA_INFO_M060000013A(data, props.auth.token);
      if (res.status.code === "S200") {
        setLoading(false);
  ...
  const save = async (data) => {
    try {
      const body = {
        ...
        laneId: data.laneId ? data.laneId : null,
        paymentmethodId: data.paymentmethodId ? data.paymentmethodId : null,
        trxNumber: data.trxNumber ? data.trxNumber : null,
        fareClass: data.fareClass ? data.fareClass : null,
        auditAdjustReasonId: data.auditAdjustReasonId ? data.auditAdjustReasonId : null,
        remark: data.remark ? data.remark : null,
        vehicleClass: data.vehicleClass === "4 ล้อ" ? 1 : 2,
      };
      const res = await POST_SAVE_M060000013A(
        body,
        props.auth.token
      );
      if (res.status.code === "S200") {
  ...
  const fareClass = async (data) => {
    try {
      ...
      const res = await POST_FARECLASS_M060000013A(
        body,
        props.auth.token
      );
      if (res.status.code === "S200") {
        form.setFieldsValue({ fareClass: res.fareClass });
      }
    } catch (error) {
      console.log(error);
    }
  ...
  const onChangeFareClass = () => {
    fareClass(form.getFieldsValue());
  };
              ...
              <Row gutter={24} justify="start" type="flex" className="w-100">
                <Col span={10}>
                  <Form.Item
                    ...
                    rules={[{
                      required: true,
                      message: "กรุณาใส่ช่องทาง!",
                    }]}
                  >
                    <Input onChange={event => onChangeFareClass(event)}
                      size={60}
                      placeholder="ช่องทาง..."
                ...
                <Col span={10}>
                  ...
                  >
                    <Select onChange={event => onChangeFareClass(event)}
                      allowClear
                      size={60}
                ...
                <Col span={10}>
                  <Form.Item
                    ...
                    rules={[
                      {
                        required: true,
                        message: "กรุณาระบุค่าผ่านทาง!",
                      },
                    ]}
                  >
                    <Input
                      size={5}
                      placeholder="รวมค่าผ่านทาง..."
                      className="rounded-pill max-WS "
                    />
                  </Form.Item>
                </Col>

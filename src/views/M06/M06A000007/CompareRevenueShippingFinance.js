...
import { GET_DATA_INFO_M060000007, GET_DATA_EXPORT_CASH_M060000007, GET_DATA_EXPORT_COUPON_M060000007, GET_DATA_BACK_TO_AUDIT_M060000007 } from "../../../service/api/report";
import { getTSBList_API } from "../../../service/api/util";
import moment from "moment";
import { header67A } from "../../../tools/excel/header";
import { footer67A } from "../../../tools/excel/footer";
...
const CompareRevenueShipping = (props) => {
    ...
    const [disabledButtonCash, setDisabledButtonCash] = useState(true);
    const [disabledButtonCoupon, setDisabledButtonCoupon] = useState(true);
            ...
            children: [
                {
                    ...
                    children: [
                        {
                            title: "V.30",
                            dataIndex: "countMccCoupon30",
                            key: "countMccCoupon30",
                            width: 40,
                            align: "center",
                            render: (text, record) => (
                                <div
                        ...
                        {
                            title: "V.70",
                            dataIndex: "countMccCoupon70",
                            key: "countMccCoupon70",
                            width: 40,
                            align: "center",
                            render: (text, record) => (
                                <div
                        ...
                        {
                            title: "V.35",
                            dataIndex: "countMccCoupon35",
                            key: "countMccCoupon35",
                            width: 40,
                            align: "center",
                            render: (text, record) => (
                                <div
                        ...
                        {
                            title: "V.80",
                            dataIndex: "countMccCoupon80",
                            key: "countMccCoupon80",
                            width: 40,
                            align: "center",
                            render: (text, record) => (
                                <div
                        ...
                        {
                            title: "Value",
                            dataIndex: "amountMccCouponValue",
                            key: "amountMccCouponValue",
                            width: 60,
                            align: "center",
                            render: (text, record) => (
                                <div
        ...
        if (item.seckey) {
            try {
                await props.history.push({
                    pathname: `/reports/compare-revenue-shipping-companies-with-system-finance/tod-detail/${item.seckey}`,
                    value: initialValue,
                });
            } catch (error) {
                console.log(error);
    ...
    const getDataInfo = async (data = null) => {
        try {
            setLoading(true);
            const res = await GET_DATA_INFO_M060000007(data, props.auth.token);
            if (res.status.code === "S200") {
                setLoading(false);
                setDataSource(res);
                setDisabledButton(res.sendToSap);
                setDisabledButtonCash(res.sendCashToSap);
                setDisabledButtonCoupon(res.sendCouponToSap);
    ...
    const handlePrint = useReactToPrint({
        ...
        documentTitle: ["6.7.1 Compare Revenue Shipping Companies With System (Cash and Coupon)"],
    });
            ...
            props: {
                ...
                onClick: () =>
                    _exportFileExcel({
                        ...
                        fileName: "6.7.1 Compare Revenue Shipping Companies With System (Cash and Coupon)",
                        header: header67A,
                        footer: footer67A,
                    }),
                // disabled: dataSource.list.length < 1,
    ...
    const ExportCashToSap = async () => {
        const datainfo = {
            date: _timeZoneThai(dateExport)
        }
        setLoading(true);
        try {
            const res = await GET_DATA_EXPORT_CASH_M060000007(datainfo, props.auth.token)
            if (res.status.code === "S200") {
                Swal.fire({
                    icon: "success",
                    title: "สำเร็จ",
    ...
    const ExportCouponToSap = async () => {
        const datainfo = {
            date: _timeZoneThai(dateExport)
        }
        setLoading(true);
        try {
            const res = await GET_DATA_EXPORT_COUPON_M060000007(datainfo, props.auth.token)
            if (res.status.code === "S200") {
                Swal.fire({
                    icon: "success",
                    title: "สำเร็จ",
    ...
    const BackToAudit = async () => {
        const datainfo = {
            date: _timeZoneThai(dateExport)
        }
        setLoading(true);
        try {
            const res = await GET_DATA_BACK_TO_AUDIT_M060000007(datainfo, props.auth.token)
            if (res.status.code === "S200") {
                Swal.fire({
                    icon: "success",
                    title: "สำเร็จ",
        ...
        <Skeleton loading={loading} active>
            ...
            <div className="d-none">
                <PrintReport
                    ...
                    header={header67A}
                    footer={footer67A}
                    columnPerPage={header67A.length}
                    propsHeader={{
                        headerText,
                        position: "d-flex justify-content-start",
                        colSpan: 12,
                        TopicText: "6.7.1 เปรียบเทียบรายได้ บริษัทขนส่ง กับ ข้อมูลจจากระบบ (Cash and Coupon) "
                    }}
            ...
            <Row justify='end' className='pt-20'>
                ...
                <Col className="ant-col text-right ant-col-md-24 ant-col-lg-5 ant-col-xl-5">
                    <Row xs={18} md={18} lg={18} style={{ marginRight: 30 }} className="text-right">
                        ...
                        <div>&nbsp;&nbsp;</div>
                        <Button type="primary" disabled={disabledButton} onClick={BackToAudit}>ส่งกลับฝ่ายตรวจสอบ</Button>                        
                    </Row>
                </Col>
                <Col className="ant-col text-right ant-col-md-24 ant-col-lg-5 ant-col-xl-5">
                            <Row xs={18} md={18} lg={18} style={{ marginLeft: 30 }}>
                                <Col className="ant-col ant-col-md-24 ant-col-lg-3 ant-col-xl-3">
                                    <Button type="primary" disabled={disabledButtonCash} onClick={ExportCashToSap}>เงินสด to SAP</Button>
                                </Col>
                            </Row>
                            <br></br>
                            <Row xs={18} md={18} lg={18} style={{ marginLeft: 30 }}>
                                <Col className="ant-col ant-col-md-24 ant-col-lg-3 ant-col-xl-3">
                                    <Button type="primary" disabled={disabledButtonCoupon} onClick={ExportCouponToSap}>คูปอง to SAP</Button>
                                </Col>
                            </Row>
                        </Col>
            </Row>

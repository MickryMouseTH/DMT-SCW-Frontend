...
import { GET_DATA_INFO_M06C000007, GET_DATA_EXPORT_M06C000007 } from "../../../service/api/report";
import { getTSBList_API } from "../../../service/api/util";
import moment from "moment";
import { header67C } from "../../../tools/excel/header";
import { footer67C } from "../../../tools/excel/footer";
...
const CompareRevenueShipping = (props) => {
        ...
        if (item.seckey) {
            try {
                await props.history.push({
                    pathname: `/reports/compare-revenue-shipping-companies-with-system-finance-emv/tod-detail/${item.seckey}`,
                    value: initialValue,
                });
            } catch (error) {
                console.log(error);
    ...
    const getDataInfo = async (data = null) => {
        try {
            setLoading(true);
            const res = await GET_DATA_INFO_M06C000007(data, props.auth.token);
            if (res.status.code === "S200") {
                setLoading(false);
                setDataSource(res);
    ...
    const handlePrint = useReactToPrint({
        ...
        documentTitle: ["6.7.3 Compare Revenue Shipping Companies With System (EMV)"],
    });
            ...
            props: {
                ...
                onClick: () =>
                    _exportFileExcel({
                        ...
                        fileName: "6.7.3 Compare Revenue Shipping Companies With System (EMV)",
                        header: header67C,
                        footer: footer67C,
                    }),
                // disabled: dataSource.list.length < 1,
    ...
    const ExportToSap = async () => {
        ...
        setLoading(true);
        try {
            const res = await GET_DATA_EXPORT_M06C000007(datainfo, props.auth.token)
            if (res.status.code === "S200") {
                Swal.fire({
                    icon: "success",
                    title: "สำเร็จ",
        ...
        <Skeleton loading={loading} active>
            ...
            <div className="d-none">
                <PrintReport
                    ...
                    header={header67C}
                    footer={footer67C}
                    columnPerPage={header67C.length}
                    propsHeader={{
                        headerText,
                        position: "d-flex justify-content-start",
                        colSpan: 12,
                        TopicText: "6.7.3 เปรียบเทียบรายได้ บริษัทขนส่ง กับ ข้อมูลจจากระบบ (EMV) "
                    }}

...
import { GET_DATA_INFO_M020000002 } from "../../../service/api/report";
...
const WorkingdayDailyAverageTrafficAndIncome = (props) => {
  ...
  const getDataInfo = async (data = null) => {
    ...
    try {
      setLoading(true);
      const res = await GET_DATA_INFO_M020000002(data, props.auth.token);
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: ["2.2 Workingday Daily Average Traffic And Income Report"]
  });
  const action = [
    {
      name: "พิมพ์",
      ...
      props: {
        ...
        onClick: () =>
          _exportFileExcel({
            ...
            fileName: "2.2 Workingday Daily Average Traffic And Income Report",
            header: header21,
            footer: footer21,
          }),
        // disabled: dataSource.list.length < 1,
        ...
        <div className="d-none">
          <PrintReport
            ...
            propsHeader={{
              ...
              colSpan: 12,
              TopicText: "2.2 ปริมาณจราจรและรายได้เฉลี่ยวันทำการ"
            }}
          />
        </div>
      </div>
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(WorkingdayDailyAverageTrafficAndIncome);

  ...
  return (
    <>
      <Table.Summary.Row className="bg_default">
        ...
        {props.record.filter((item, ind) => ind !== 0 && ind !== 1).map((item, ind) => (   //resolve =>> Cannot read property '0' of undefined
          <React.Fragment key={ind}>
            ...
            <Table.Summary.Cell colSpan={1} className="text-right">
              ...
              <Text type="secondary" align="right" >{_isNull(Number(columnList[item['children'][1]['key']]))}</Text>
            </Table.Summary.Cell>
          </React.Fragment>
        ))}
      </Table.Summary.Row>

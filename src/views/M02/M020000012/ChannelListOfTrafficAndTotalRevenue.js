...
import { GET_DATA_INFO_M020000012 } from "../../../service/api/report";
import moment from "moment";
const { Text } = Typography;
const dateFormat = "DD/MM/YYYY";

const ChannelListOfTrafficAndTotalRevenue = (props) => {
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: ["2.12 Hourly Traffic And Income By Lane Report"],
  });
  ...
  const OnSearchDataInfo = async (data = null) => {
    try {
      setLoadingDataSource(true);
      const res = await GET_DATA_INFO_M020000012(data, props.auth.token);
      if (res.status.code === "S200") {
        setDataSource(res);
        setLoadingDataSource(false);
      } else {
      ...
      props: {
        ...
        onClick: () =>
          _exportFileExcel({
            ...
            fileName: "2.12 Hourly Traffic And Income By Lane Report",
            header: header41,
            footer: footer41,
          }),
        // disabled: dataSource.list.length < 1,
        ...
        <div className="d-none">
          <PrintReport
            ...
            propsHeader={{
              ...
              colSpan: 8,
              TopicText: "2.12 ปริมาณจราจรและรายได้รวม รายชั่วโมง จำแนกรายด่าน รายช่องทาง",
            }}
          />
        </div>
      </div>
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(ChannelListOfTrafficAndTotalRevenue);

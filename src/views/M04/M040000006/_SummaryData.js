    ...
    let datacolumn = rowsData.reduce((accumulator, currentValue) => {
      let newacc = [...accumulator.map(itemAcc => {
        ...
        itemAcc['children'].forEach(el => {
          if (el.type === 'traffic') newitem.traffic = (Number(currentValue[el.key]) + Number(newitem.traffic)).toFixed(2)
          if (el.type === 'revenue') newitem.revenue = (Number(currentValue[el.key]) + Number(newitem.revenue)).toFixed(2)
        });
        return newitem
      })]
      return newacc
        ...
        {columnList.map((item, ind) => (
          <React.Fragment key={ind}>
            <Table.Summary.Cell colSpan={1} className="text-center">
              <Text type="secondary" align="center" >{_isNull(Number(item['traffic']))}</Text>
            </Table.Summary.Cell>
            <Table.Summary.Cell colSpan={1} className="text-center">
              <Text type="secondary" align="center" >{_isNull(Number(item['revenue']))}</Text>
            </Table.Summary.Cell>
          </React.Fragment>
        ))}
      </Table.Summary.Row>

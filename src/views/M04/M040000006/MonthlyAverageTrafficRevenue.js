...
import { header46, footer46 } from "../../../tools/excel";
import { getPlazaListAPI } from "../../../service/api/util";
import { GET_DATA_INFO_M04000006 } from "../../../service/api/report";

const { Text } = Typography;
const dateFormat = "YYYY";

const MonthlyAverageTrafficRevenue = (props) => {
  ...
  const getDataInfo = async (data = null) => {
    try {
      setLoadingDataSource(true);
      const res = await GET_DATA_INFO_M04000006(data, props.auth.token);
      if (res.status.code === "S200") {
        setDataSource(res);
        setLoadingDataSource(false);
      } else {
  ...
  const handleOnFinish = (value) => {
    ...
    const year = moment(value.year).format(dateFormat);
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: ["4.6 Monthly Average Traffic Revenue Report"],
  });
      ...
      props: {
        ...
        onClick: () =>
          _exportFileExcel({
            ...
            fileName: "4.6 Monthly Average Traffic Revenue Report",
            header: header46(_setYearThai(yearTitle,"YYYY")),
            footer: footer46,
          }),
        // disabled: dataSource.list.length < 1,
      },
        ...
        <div className="d-none">
          <PrintReport
            ...
            propsHeader={{
              ...
              colSpan: 8,
              TopicText: "4.6 ปริมาณจราจร/รายได้เฉลี่ยรายเดือน"
            }}
          />
        </div>
      </div>

...
import { GET_DATA_INFO_M04000007 } from "../../../service/api/report";

const dateFormat = "YYYY";
const { Text } = Typography;

const YearlyAverageTrafficRevenue = (props) => {
  ...
  const fields = [
    ...
    },
    {
      ...
      option: {
        ...
        rules: [
          { required: true, message: "กรุณาป้อนจำนวนปีย้อนหลัง!" },
  ...
  const getDataInfo = async (data = null) => {
    try {
      setLoadingDataSource(true);
      const res = await GET_DATA_INFO_M04000007(data, props.auth.token);
      if (res.status.code === "S200") {
        setDataSource(res);
        setLoadingDataSource(false);
      } else {
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: ["4.7 Yearly Average Traffic Revenue Report"],
  });
  const action = [
    {
      name: "พิมพ์",
      ...
      props: {
        ...
        onClick: () =>
          _exportFileExcel({
            ...
            fileName: "4.7 Yearly Average Traffic Revenue Report",
            dataSource: {
        ...
        <div className="d-none">
          <PrintReport
            ...
            propsHeader={{
              ...
              colSpan: 12,
              TopicText: "4.7 ปริมาณจราจร/รายได้เฉลี่ยรายปี"
            }}
          />
        </div>
      </div>

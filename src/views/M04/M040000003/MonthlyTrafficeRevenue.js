...
import { header43, footer43 } from "../../../tools/excel";
import { getPlazaListAPI } from "../../../service/api/util";
import { GET_DATA_INFO_M04000003 } from "../../../service/api/report";
...
const MonthlyTrafficeRevenue = (props) => {
  ...
  const fields = [
    ...
    },
    {
      ...
      option: {
        ...
        rules: [{ required: true, message: "กรุณาเลือกวันปี!" }],
  ...
  const headerText = [
    ...
    },
    {
      ...
      value: dataToPrint.DataList
        ? _setYearThai(dataToPrint.DataList.year,"YYYY")
        : "",
    },
  ];
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: ["4.3 Monthly Traffic Revenue Report"],
  });
  ...
  const getDataInfo = async (data = null) => {
    try {
      setLoadingDataSource(true);
      const res = await GET_DATA_INFO_M04000003(data, props.auth.token);
      if (res.status.code === "S200") {
        setDataSource(res);
        setLoadingDataSource(false);
      } else {
  ...
  const handleOnFinish = (value) => {
    ...
    const year = _setYearThai(value.year,dateFormat);
      ...
      props: {
        ...
        onClick: () =>
          _exportFileExcel({
            ...
            fileName: "4.3 Monthly Traffic Revenue Report",
            header: header43(_setYearThai(yearTitle,"YYYY")),
            footer: footer43,
          }),
        // disabled: dataSource.list.length < 1,
      },
        ...
        <div className="d-none">
          <PrintReport
            ...
            propsHeader={{
              ...
              colSpan: 8,
              TopicText: "4.3 ปริมาณจราจร/รายได้รายเดือน",
            }}
          />
        </div>
      </div>

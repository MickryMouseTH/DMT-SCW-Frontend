...
import { GET_DATA_INFO_M040000002 } from "../../../service/api/report";
import moment from "moment";
const { Text } = Typography;
const dateFormat = "DD/MM/YYYY";

const DailyTrafficRevenue = (props) => {
      ...
      column.children = columnH.children.map((ch) => {
        ...
        if (ch.title === "ปริมาณ") {
          chItem.render = (text, record) => {
            ...
            return (
              <span>
                {_isEmpty(traffic) ? null : _isNull(traffic["traffic"])}
              </span>
            );
          };
        }
        if (ch.title === "รายได้") {
          chItem.render = (text, record) => {
            ...
            return (
              <span>
                {_isEmpty(traffic) ? null : _isNull(traffic["revenue"])}
              </span>
            );
          };
        }
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: ["4.2 Daily Traffic Revenue Report"],
  });
  ...
  const OnSearchDataInfo = async (data = null) => {
    try {
      setLoadingDataSource(true);
      const res = await GET_DATA_INFO_M040000002(data, props.auth.token);
      if (res.status.code === "S200") {
        setDataSource(res);
        setLoadingDataSource(false);
      } else {
      ...
      props: {
        ...
        onClick: () =>
          _exportFileExcel({
            ...
            fileName: "4.2 Daily Traffic Revenue Report",
            dataSource: {
        ...
        <div className="d-none">
          <PrintReport
            ...
            propsHeader={{
              ...
              colSpan: 12,
              TopicText: "4.2 ปริมาณจราจร/รายได้รายวัน",
            }}
          />
        </div>
      </div>

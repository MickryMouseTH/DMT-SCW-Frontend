...
import { UPLOAD_FILE_ETC_M060000015 } from "../../../service/api/report";
...
const ImportFileJsonTransaction = (props) => {
    ...
    const fileHandlerEtc = fileList => {
        ...
        if (!_isEmpty(fileObj)) {
            if (fileObj.type === "application/json") {
                ...
                message.error('อัปโหลดไฟล์ไม่สำเร็จ กรุณาอัปโหลดไฟล์ json (.json) เท่านั้น ');
                return false;
            }
        } else {
            return false;
    ...
    const handleSubmitFileEtc = async () => {
        try {
            ...
            const res = await UPLOAD_FILE_ETC_M060000015(formData, props.auth.token)
    ...
    return (
        <Skeleton loading={loading} active>
            <Row className='d-flex justify-content-center mt-10'>
                <Text style={{ fontSize: '25px', fontWeight: 'bold' }}>นำเข้าข้อมูลส่งตัด CS</Text>
            </Row>

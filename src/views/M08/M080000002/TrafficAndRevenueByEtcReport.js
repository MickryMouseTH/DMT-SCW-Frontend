...
import { GET_DATA_INFO_M080000002 } from "../../../service/api/report";
...
const TrafficAndRevenueByEtcReport = (props) => {
  ...
  const columnsOne = [
    ...
    },
    {
      ...
      render: (value, row, index) => {
          ...
          if (row.mpassRevenue === "") { obj.props.colSpan = 0; obj.props.rowSpan = 1; }
          else { obj.props.colSpan = 1; obj.props.rowSpan = 1; }
          return obj;
      } 
    },
    {
      ...
      render: (value, row, index) => {
          ...
          if (row.easyPassRevenue === "") { obj.props.colSpan = 0; obj.props.rowSpan = 1; }
          else { obj.props.colSpan = 1; obj.props.rowSpan = 1; }
          return obj;
      } 
    },
    {
      ...
      render: (value, row, index) => {
          ...
          if (row.totalRevenue === "") { obj.props.colSpan = 0; obj.props.rowSpan = 1; }
          else { obj.props.colSpan = 1; obj.props.rowSpan = 1; }
          return obj;
      } 
    },
...
//   ]
    ...
    if (item.seckey) {
      try {
        await props.history.push({
          pathname: `/traffic-and-revenue-by-etc/plaza-detail/${item.seckey}`,
          value: initialValue,
        });
      } catch (error) {
        console.log(error);
  ...
  const addIndex = (res) => {
    ...
    const totalAll = {
        ...
        mpassRevenue: res.sumMPassRevenue,
        easyPassRevenue: res.sumEasyPassRevenue,
        totalRevenue: res.sumTotalRevenue,
    }
  ...
  const getDataDailyTollCollction = async (dataOutput) => {
    try {
      setLoading(true);
      const res = await GET_DATA_INFO_M080000002(dataOutput, props.auth.token);
      console.log("res 8.2", res);
      if (res.status.code === "S200") {
        setLoading(false);

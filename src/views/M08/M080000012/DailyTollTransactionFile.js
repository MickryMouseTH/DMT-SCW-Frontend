...
import { CREATE_FILE_M080000012, DOWNLOAD_FILE_M080000012 } from "../../../service/api/report";
...
const ReportEtcForTaxInvoice = (props) => {
  ...
  const handleDownload = async (item) => {
    if (item.pathName) {
      ...
      try {
        await DOWNLOAD_FILE_M080000012(sendItem, props.auth.token);
      } catch (error) {
        console.log(error);
      }
    } else {
  ...
  const getDataList = async (dataOutput) => {
    try {
      setLoading(true);
      const res = await CREATE_FILE_M080000012(dataOutput, props.auth.token);

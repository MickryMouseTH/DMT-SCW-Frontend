...
import { GET_PLAZA_DETAIL_M080000001 } from "../../../service/api/report";
...
const PlazaDetail = (props) => {
    ...
    const handleDetail = async (item) => {
        ...
        try {
            await props.history.push({
              pathname: `/traffic-by-etc/lane-detail/${item.seckeyPlaza}`,
              value: initialValue,
            });
        } catch (error) {
            console.log(error);
  ...
  const getPlazaDetail = async () => {
    try {
        ...
        const res = await GET_PLAZA_DETAIL_M080000001(
          body,
          props.auth.token
        );

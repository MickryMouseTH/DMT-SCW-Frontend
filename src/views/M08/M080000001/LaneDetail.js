...
import { GET_LANE_DETAIL_M080000001 } from "../../../service/api/report";
...
const LaneDetail = (props) => {
    ...
    const handleDetail = async (item) => {
        ...
        try {
            await props.history.push({
              pathname: `/traffic-by-etc/hour-detail/${item.seckeyLane}`,
              value: initialValue,
            });
        } catch (error) {
            console.log(error);
  ...
  const getLaneDetail = async () => {
    try {
        ...
        const res = await GET_LANE_DETAIL_M080000001(
          body,
          props.auth.token
        );
    ...
    const goBack = () => {
        props.history.push({
            pathname: `/traffic-by-etc/plaza-detail/${dataSource.backPlazaPath}`,

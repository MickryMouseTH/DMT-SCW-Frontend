...
import { GET_DATA_INFO_M010000003 } from "../../../service/api/report";
...
const LaneOperatingStatisticsReports = (props) => {
  ...
  const getDataInfo = async (data = null) => {
    ...
    try {
      setLoading(true);
      const res = await GET_DATA_INFO_M010000003(data, props.auth.token);
      if (res.status.code === "S200") {
  ...
  const handleDetail = async (item) => {
    ...
    try {
      await props.history.push({
        pathname: `/reports/top-5-statistical-opening-hours/reportdetail/${item.laneId}`,
        state: { ...sendItem },
        value: initialValue,
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: ["1.3 Top 5 Statistical Opening Hours"],
  });
      ...
      props: {
        ...
        onClick: () =>
          _exportFileExcel({
            ...
            fileName: "1.3 Top 5 Statistical Opening Hours",
            header: header11,
          }),
        // disabled: dataSource.list.length < 1,
      },
      ...
      <div className="d-none">
        <PrintReport
          ...
          propsHeader={{
            ...
            colSpan: 12,
            TopicText: "1.3 5 อันดับ ของสถิติจำนวนชั่วโมงการเปิด-ปิด และการหยุดใช้งานของช่องทาง จำแนกรายด่าน รายช่องทาง"
          }}
        />
      </div>
    </Skeleton>

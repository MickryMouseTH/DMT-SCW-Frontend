...
import { GET_DATA_INFO_M010000004 } from "../../../service/api/report";
...
const LaneAlarmStatisticsReports = (props) => {
  ...
  const getDataInfo = async (data = null) => {
    ...
    try {
      setLoading(true);
      const res = await GET_DATA_INFO_M010000004(data, props.auth.token);
      if (res.status.code === "S200") {
  ...
  const handleDetail = async (item) => {
    ...
    try {
      await props.history.push({
        pathname: `/reports/top-5-lane-alarm-statistics-reports/reportdetail/${item.laneId}`,
        state: { ...sendItem },
        value: initialValue,
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: ["1.4 Top 5 Lane Alarm Statistics Reports"],
  });
      ...
      props: {
        ...
        onClick: () =>
          _exportFileExcel({
            ...
            fileName: "1.4 Top 5 Lane Alarm Statistics Reports",
            header: header12,
          }),
        // disabled: dataSource.list.length < 1,
      },
      ...
      <div className="d-none">
        <PrintReport
          ...
          propsHeader={{
            ...
            colSpan: 12,
            TopicText: "1.4 5 อันดับ สถิติการเกิดข้อผิดพลาดของการใช้งานแต่ละช่องทาง"
          }}
        />
      </div>
    </Skeleton>

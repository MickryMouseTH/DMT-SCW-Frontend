...
import { GET_DATA_INFO_M09000004 } from "../../../service/api/report";
import { 
  // _exportFileExcel, 
  // _timeZoneThai,
  //  _isNull,
...
const TariffStatus = (props) => {
  ...
  const columns = [
    ...
    },
    {
      title: "hqRevision",
      key: "hqRevision",
      dataIndex: "hqRevision",
      width: 20,
      align: "center",
      // render: (text) => _isNull(text)
      render: (text) => checkValue(text)
    },
    {
      title: "Lane_1st",
      fixed: "right",
  ...
  const getDataInfo = async (data = null) => {
    try {
      setLoadingDataSource(true);
      const res = await GET_DATA_INFO_M09000004(data, props.auth.token);
      console.log("4.5", res);
      if (res.status.code === "S200") {
        setDataSource(res);
        setLoadingDataSource(false);
...
export default connect(mapStateToProps, mapDispatchToProps)(TariffStatus)

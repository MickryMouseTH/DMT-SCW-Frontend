...
import { GET_DATA_DETAIL_M050000005 } from '../../../service/api/report'
...
const ReportDetail = (props) => {
  ...
  const goBack = () => {
    props.history.push({
      pathname: `/reports/maintenance-tod-reports`,
  ...
  const getDataInfo = async (data = null) => {
    ...
    try {
      setLoading(true)
      const res = await GET_DATA_DETAIL_M050000005(dataDetail, props.auth.token)
        ...
        <Card className="w-60">
          ...
          <Row className="d-flex justify-content-between mb-20 mt-5">
            ...
            <Col span={12} className="pl-20">
              ...
              <Row className="d-flex justify-content-between mb-5">
                <Col span={12}>กะ</Col>
                <Col span={12}>{dataSource.shiftType === 'SHIFT 1' ? 'เช้า' : dataSource.shiftType === 'SHIFT 2' ? 'บ่าย' : 'เย็น'}</Col>
              </Row>
              <Row className="d-flex justify-content-between mb-5">

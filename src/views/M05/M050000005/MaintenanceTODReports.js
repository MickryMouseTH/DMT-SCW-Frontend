...
import { GET_DATA_INFO_M050000005 } from "../../../service/api/report";
...
const TODReports = (props) => {
    ...
    if (item.seckey) {
      ...
      try {
        await props.history.push({
          ...
          pathname: `/reports/maintenance-tod-reports/report-detail`,
          state: { ...sendItem },
          value: initialValue,
  ...
  const addIndex = (res) => {
    const list = res.list.map((item, index) => {
      ...
      return {
        ...
        , shiftTypeTh: _shift[item.shiftTypeName] ? _shift[item.shiftTypeName] : ""
      }
    })
  ...
  const getDataInfo = async (data = null) => {
    try {
      setLoading(true);
      const res = await GET_DATA_INFO_M050000005(data, props.auth.token);
      if (res.status.code === "S200") {
        setLoading(false);
        // setDataSource(res);
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: ["5.5 Maintenance Tod Report"],
  });
      ...
      props: {
        ...
        onClick: () =>
          _exportFileExcel({
            ...
            fileName: "5.5 Maintenance Tod Report",
            header: header36,
            footer: footer36,
          }),
        // disabled: dataSource.list.length < 1,
      ...
      <div className="d-none">
        <PrintReport
          ...
          propsHeader={{
            ...
            colSpan: 12,
            headerText,
            TopicText: "5.5 รายงานการนำส่งเงินโหมดบำรุงรักษา",
          }}

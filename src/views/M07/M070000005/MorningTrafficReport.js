...
import { GET_DATA_INFO_M070000005 } from "../../../service/api/report";
...
const MorningTrafficReport = (props) => {
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: ["7.5 Morning Traffic Reports"],
  });
      ...
      props: {
        ...
        onClick: () =>
          _exportFileExcel({
            ...
            fileName: "7.5 Morning Traffic Reports",
            header: header75,
          }),
      },
    },
  ...
  const getDataDailyTollCollction = async (dataOutput) => {
    try {
      setLoading(true);
      const res = await GET_DATA_INFO_M070000005(dataOutput, props.auth.token);
      console.log("res", res)
      if (res.status.code === "S200") {
        setLoading(false);
      ...
      <div className="d-none">
        <PrintPDF ref={printReportRef}
          ...
          HeaderBar={{
            ...
            colSpan: 8,
            headerText,
            TopicText: "7.5 รายงานการจราจรตอนเช้า",
          }}
        />
      </div>
    </Skeleton>
...
export default connect(mapStateToProps, mapDispatchToProps)(MorningTrafficReport)

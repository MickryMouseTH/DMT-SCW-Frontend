...
const Print = ({
        ...
        html = [
          ...
          <div key={`${sourceIndex}${headerIndex}`} className="m-10">
            ...
            <Table
              ...
              className={sourceIndex === source.length - 1 ? `print-size6 print-border ${propsClass}` : `print-size6 print-border`}
              pagination={false}
              columns={_columns}
              summary={null}
            />
  ...
  const _splitHeader = () => {
    ...
    const prefix0 = header[0];
    const prefix1 = header[1];
    const prefix2 = header[2];
    let result = [];
    let start = 1;
    for (let i = 0; i < length-1; i++) {
      const _split = i===1? [...header].splice(start, columnPerPage+5) :  [...header].splice(start, columnPerPage);
      if(i===0){
        result = [...result, [prefix0, ..._split]];
      }else{
        result = [...result, [prefix0, prefix1, prefix2, ..._split]];
      }
      start += columnPerPage;
    }
    if (columnPerPage <= 1) {
  ...
  const reformatFooter = (header = [], footer = [], dataSource = []) => {
      ...
      if (headerItem.children && headerItem.children.length) {
        headerItem.children.forEach((childrenItem) => {
          if (childrenItem.children && childrenItem.children.length) {
            childrenItem.children.forEach((childrenItem2) => {
              reforgeHeader = [...reforgeHeader, childrenItem2];
            });
          } else {
    ...
    if (oneColumnfooter === true) {
      ...
      if (_isEmpty(columnTotalChange)) {
        return { ...result, plazaAbbreviation: "", laneAbbreviation: "รวม" };
      } else {

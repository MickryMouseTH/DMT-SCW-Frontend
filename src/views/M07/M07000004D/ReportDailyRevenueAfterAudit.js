...
import { GET_DATA_INFO_M07000004D } from "../../../service/api/report";
...
const ReportDailyRevenueAfterAudit = (props) => {
  ...
  const getDataInfo = async (data = null) => {
    try {
      setLoading(true);
      const res = await GET_DATA_INFO_M07000004D(data, props.auth.token);
      if (res.status.code === "S200") {
        setLoading(false);
        setDataSource(res);
      } else {
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: ["7.4.4 AVERAGE DAILY REVENUE"],
  });
      ...
      props: {
        ...
        onClick: () => {
          _exportFileExcel({
            ...
            fileName: "7.4.4 AVERAGE DAILY REVENUE",
            header: headerExcel,
          });
        },
      },
        ...
        <div className="d-none">
          <PrintPDF
            ...
            HeaderBar={{
              ...
              colSpan: 24,
              TopicText: "7.4.4 AVERAGE DAILY REVENUE"
            }}
          />
        </div>
      </div>

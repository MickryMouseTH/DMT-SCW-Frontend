...
import { GET_DATA_INFO_M07000005D } from "../../../service/api/report";
...
const ReportDailyTrafficAfterAudit = (props) => {
  ...
  const getDataInfo = async (data = null) => {
    try {
      setLoading(true);
      const res = await GET_DATA_INFO_M07000005D(data, props.auth.token);
      if (res.status.code === "S200") {
        setLoading(false);
        setDataSource(res);
      } else {
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: ["7.5.4 AVERAGE DAILY TRAFFIC"],
  });
      ...
      props: {
        ...
        onClick: () => {
          _exportFileExcel({
            ...
            fileName: "7.5.4 AVERAGE DAILY TRAFFIC",
            header: headerExcel,
          });
        },
      },
        ...
        <div className="d-none">
          <PrintPDF
            ...
            HeaderBar={{
              ...
              colSpan: 37,
              TopicText: "7.5.4 AVERAGE DAILY TRAFFIC"
            }}
          />
        </div>
      </div>
